...
import QuServerBuilder.jacksonSimpleServerBuilder
import AbstractQuService.{ServiceFactory, jacksonSimpleQuorumServiceFactory}
...
object QuServer {
  ...
  def builder[U:TypeTag](myServerInfo: RecipientInfo, thresholds: QuorumSystemThresholds, obj: U): Unit =
    jacksonSimpleServerBuilder[U](myServerInfo, thresholds, obj, myServerInfo.port)
}
...
// - tls support
class QuServerImpl[Marshallable[_], U](quService: AbstractQuService[Marshallable, U], port: Int) extends QuServer {
...
class QuServerBuilder[Marshallable[_], U](private val serviceFactory: ServiceFactory[Marshallable, U],
                                          //also interceptor is a dependency: SimpleJwtInterceptor
                                          //user (injected) dependencies:
                                          private val myServerInfo: RecipientInfo,
                                          private val quorumSystemThresholds: QuorumSystemThresholds,
                                          private val obj: U,
                                          private val port: Int) {

  private val quService: AbstractQuService[Marshallable, U] = serviceFactory(quorumSystemThresholds, myServerInfo, obj)
  ...
  def addServer(serverInfo: RecipientInfo): QuServerBuilder[Marshallable, U] = {
  ...
  def build(): QuServer = {
    ...
    new QuServerImpl(quService, port)
...
object QuServerBuilder {
  ...
  def jacksonSimpleServerBuilder[U: TypeTag](myServerInfo: RecipientInfo,
                                             ...
                                             obj: U,
                                             port: Int) =
    new QuServerBuilder[JavaTypeable, U](jacksonSimpleQuorumServiceFactory(), myServerInfo, thresholds, obj, port)
